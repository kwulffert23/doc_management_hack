...
agent.predict(
    None,
    ChatCompletionRequest(
        messages=[Message(role="user", content="What are the financial incentives for Distribution Network Operators under RIIO-ED2?")]
    ),
)
...
agent.predict(
    None,
    ChatCompletionRequest(
        messages=[Message(role="user", content="what is the LAEP used for in the DSO Incentive Report?")]
    ),
)
...
agent.predict(
    None,
    ChatCompletionRequest(
        messages=[Message(role="user", content="What is DSO?")]
    ),
)
